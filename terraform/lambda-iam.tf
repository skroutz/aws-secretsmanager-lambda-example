...
data "aws_iam_policy_document" "lambda-policy" {
    statement {
        ...
        resources = [
            # All secrets under "secrets-example/" path are Readable with this Permission
            "arn:aws:secretsmanager:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:secret:${local.secret-path}/*",
